...
changed slightly to allow new paths and checks if enough arguments
"""
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 4:
        print("Error, to little arguments, neccesary is observer initials AND Image Number AND Repetition Number!")
    else:
        observer = sys.argv[1]
        image = sys.argv[2]
        rep = sys.argv[3]
            
        designfile = "output/%s/%s/design_%s_triads_%s.csv" %(image, observer, observer, rep)
