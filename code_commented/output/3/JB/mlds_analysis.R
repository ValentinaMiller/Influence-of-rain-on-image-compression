...
setwd("~/Uni/visuell/imagequalityseminar/code/output/3/JB/")
...
for (i in 0:2){
	...
	if(i==0){# condition artificial rain 
		d1 <- read.csv('design_JB_triads_0_results_art_rain.csv')
		d2 <- read.csv('design_JB_triads_1_results_art_rain.csv')
		d3 <- read.csv('design_JB_triads_2_results_art_rain.csv')
		}
	...
	if(i==1){# condition rain 
		d1 <- read.csv('design_JB_triads_0_results_rain.csv')
		d2 <- read.csv('design_JB_triads_1_results_rain.csv')
		d3 <- read.csv('design_JB_triads_2_results_rain.csv')
		}

	if(i==2){#  condition no rain
		d1 <- read.csv('design_JB_triads_0_results_norain.csv')
		d2 <- read.csv('design_JB_triads_1_results_norain.csv')
		d3 <- read.csv('design_JB_triads_2_results_norain.csv')
		}
	...
	stim <- c(0.5, 0.58, 0.66, 0.74, 0.82, 0.9)
	...
	if(max(scale[[1]])!=0){#  condition no rain
		scale[[1]]=scale[[1]]/max(scale[[1]])
		}
	else{#  condition no rain
		scale[[1]]=scale[[1]]/max((-1*scale[[1]]))
		}
	print(scale)
	...
	if(i==0){#  condition no rain
		pdf('scale_one_observer_art_rain.pdf')
		plot(scale, xlab="Komprimierungsgrade", ylab="Perceptual scale", col="red")
		}
		
	if(i==1){#  condition no rain
		scale1=scale
		#pdf('scale_one_observer_rain.pdf')
		points(scale, xlab="Komprimierungsgrade", ylab="Perceptual scale", col="blue")
		}

	if(i==2){#  condition no rain
		#pdf('scale_one_observer_norain.pdf')
		points(scale, xlab="Komprimierungsgrade", ylab="Perceptual scale",col="green")
		}
}
dev.off()
# END
